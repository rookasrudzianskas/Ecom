...
const Header = ({}) => {
  ...
  const createClerkPasskey = async () => {
    try {
      ...
      console.log('Passkey created:', response.data.passkey);
    } catch (err) {
  ...
  return (
    <header className={'flex flex-wrap justify-between items-center px-4 py-2'}>
      <div className={'flex w-full flex-wrap justify-between items-center'}>
        ...
        >
          <input type={'text'} name={'query'} placeholder={'Search for products'} className={'bg-gray-100' +
            ' text-gray-800 px-4 py-2 mt-5 rounded focus:outline-none focus:ring-2 focus:ring-blue-500' +
            ' focus:ring-opacity-50 border w-full max-w-4xl'}/>
        </Form>

        <div className={'flex items-center space-x-4 mt-4 sm:mt-0 flex-1 sm:flex-none'}>
          <Link href={'/basket'} className={'flex-1 relative flex justify-center sm:justify-start sm:flex-none' +
            ...
            <span className={'absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center' +
              ' justify-center text-xs'}>{itemCount}</span>
            <span>My Basket</span>
          </Link>
          ...
          <ClerkLoaded>
            ...
            {user?.passkeys.length === 0 && (
              <button
                ...
                className={'bg-white hover:bg-blue-700 hover:text-white animate-pulse text-blue-500 font-bold py-2' +
